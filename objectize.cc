...
EMSCRIPTEN_KEEPALIVE void doNoise3(int seed, float baseHeight, float *freqs, int *octaves, float *scales, float *uvs, float *amps, int dims[3], float shifts[3], float offset, float *potential, unsigned int *colors) {
  noise3(seed, baseHeight, freqs, octaves, scales, uvs, amps, dims, shifts, offset, potential, colors);
}
